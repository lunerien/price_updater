...
class TopBar(BoxLayout):
    def __init__(self, scrollapp:ScrollApp, right_side:Menu, **kwargs):
        ...
        self.change_loc_button = Button(text=language.get_text(Text.CHANGE_XLSX_WORKBOOK.value), size_hint=(0.45, 1), background_color=TOP_BAR_COLOR)
        self.add_new_coin_button = Button(text=language.get_text(Text.ADD_NEW_COIN.value), size_hint=(0.45, 1), background_color=TOP_BAR_COLOR)
        ...
        self.language_button = Button(text=language.get_current_language(), size_hint =(0.1, 1), pos =(350, 300), background_color=TOP_BAR_COLOR)
        self.language_button.bind(on_release = self.language_list_buttons.open)
        self.language_list_buttons.bind(on_select = lambda instance, x: setattr(self.language_button, 'text_language', x))
