        ...
        if ticker in fiat_assets:
                ...
                match (ticker):
                    case "usd":
                        return {
                            ...
                            Currency.EUR: str(
                                round(currency.usd_pln / currency.eur_pln, dec)
                            ).replace(".", ","),
                            Currency.GBP: str(
                                round(currency.usd_pln / currency.gbp_pln, dec)
                            ).replace(".", ","),
                        }
                    case "gbp":
                        return {
                            Currency.USD: str(
                                round(currency.gbp_pln / currency.usd_pln, dec)
                            ).replace(".", ","),
                            Currency.PLN: str(currency.gbp_pln).replace(".", ","),
                            Currency.EUR: str(
                                round(currency.gbp_pln / currency.eur_pln, dec)
                            ).replace(".", ","),
                            Currency.GBP: "1,0",
                        }
                    case "eur":
                        return {
                            Currency.USD: str(
                                round(currency.eur_pln / currency.usd_pln, dec)
                            ).replace(".", ","),
                            Currency.PLN: str(currency.eur_pln).replace(".", ","),
                            Currency.EUR: "1,0",
                            Currency.GBP: str(
                                round(currency.eur_pln / currency.gbp_pln, dec)
                            ).replace(".", ","),
                        }
            except:
                {
            ...
            try:
                return {
                    ...
                    Currency.PLN: str(
                        round(price_exact * currency.usd_pln, dec)
                    ).replace(".", ","),
                    Currency.EUR: str(
                        round(price_exact * (currency.usd_pln / currency.eur_pln), dec)
                    ).replace(".", ","),
                    Currency.GBP: str(
                        round(price_exact * (currency.usd_pln / currency.gbp_pln), dec)
                    ).replace(".", ","),
                }
            except:
                return (
            ...
            try:
                return {
                    Currency.USD: str(
                        round(price_exact * (currency.gbp_pln / currency.usd_pln), dec)
                    ).replace(".", ","),
                    Currency.PLN: str(
                        round(price_exact * currency.gbp_pln, dec)
                    ).replace(".", ","),
                    Currency.EUR: str(
                        round(price_exact * (currency.gbp_pln / currency.eur_pln), dec)
                    ).replace(".", ","),
